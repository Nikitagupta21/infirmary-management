     ...
     //(FirstName,LastName,ID,DOB,Designation) 
        ...
        //   String query1 = "UPDATE salary SET BS='"+salary+"' WHERE EmployeeID='"+id+"'";
           System.out.println(ltype);
            PreparedStatement ps=con.prepareStatement("select * from `leave1` WHERE userName=? and month=? and year=?");
            PreparedStatement ps1=con.prepareStatement("UPDATE `leave1` SET "+ltype+"=? WHERE userName=? and month=? and year=?");
...
// ps.setString(2,fname);
    ...
    //        ps.setDate(9,doj);
       ...
       //     System.out.println(i);
            ...
            if(i!=0)
             response.sendRedirect("welcomeemployee.jsp?userName=Leave Applied Successfully !!");
                else
